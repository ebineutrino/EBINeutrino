...
public class EBIDataControlAccountStack {
    ...
    public EBIDataControlAccountStack() {
    ...
    public boolean dataStore() {
        try {
            ...
            actStack.setAccountType(EBISystem.getCRMModule().getAccountPane().accountDebitCreditType);
            actStack.setAccountTaxType(EBISystem.getCRMModule().getAccountPane().accountDebitTaxName);
    ...
    public void dataEdit(final int id) {
        ...
        try {
            ...
            if (iter.hasNext()) {
                ...
                EBISystem.getCRMModule().getAccountPane().accountDebitCreditType = actStack.getAccountType();
                EBISystem.getCRMModule().getAccountPane().accountDebitTaxName = actStack.getAccountTaxType();
                ...
                EBISystem.gui().table("accountTable", "Account").changeSelection(
                        EBISystem.gui().table("accountTable", "Account")
                                .convertRowIndexToView(EBISystem.getCRMModule().dynMethod
                                        .getIdIndexFormArrayInATable(EBISystem.getCRMModule().getAccountPane().tabModAccount.data, 6, id)),
                        0, false, false);
            } else {
    ...
    public void dataNew() {
        accountID = -1;
        EBISystem.getCRMModule().getAccountPane().accountDebitCreditType = 0;
        EBISystem.getCRMModule().getAccountPane().accountDebitTaxName = "";
        EBISystem.getCRMModule().getAccountPane().showCreditID = -1;
        EBISystem.getCRMModule().getAccountPane().showDebitID = -1;
        EBISystem.gui().vpanel("Account").setID(-1);
        actStack = new Accountstack();
        EBISystem.getInstance().getDataStore("Account", "ebiNew");
        EBISystem.getCRMModule().getAccountPane().initialize(false);
    }
    ...
    public void dataShow(final String invoiceYear) {
        ...
        try {
            ...
            if (query.list().size() > 0) {
                final Iterator iter = query.iterate();
                EBISystem.getCRMModule().getAccountPane().tabModAccount.data = new Object[query.list().size()][8];

                int i = 0;
                while (iter.hasNext()) {
                    final Accountstack act = (Accountstack) iter.next();
                    EBISystem.hibernate().session("EBIACCOUNT_SESSION").refresh(act);
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][0] = EBISystem.getInstance().getDateToString(act.getAccountdate());
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][1] = act.getAccount() == null ? "" : act.getAccount();
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][2] = act.getAccountnr() == null ? "" : act.getAccountnr();
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][3] = act.getAccountname() == null ? "" : act.getAccountname();
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][4] = currency.format(act.getAccountvalue()) == null ? "" : currency.format(act.getAccountvalue());
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][5] = act.getAccountDebit() == null ? "" : act.getAccountDebit();
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][6] = act.getAccountCredit() == null ? "" : act.getAccountCredit();
                    EBISystem.getCRMModule().getAccountPane().tabModAccount.data[i][7] = act.getAcstackid();
                    i++;
                }
            } else {
                EBISystem.getCRMModule().getAccountPane().tabModAccount.data = new Object[][]{
                        {EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", "", ""}};
            }
            EBISystem.getCRMModule().getAccountPane().tabModAccount.fireTableDataChanged();
            EBISystem.hibernate().transaction("EBIACCOUNT_SESSION").commit();
        } catch (final Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void createHistory(final int id) {
        ...
        try {
            EBISystem.getCRMModule().hcreator.setDataToCreate(new EBICRMHistoryDataUtil(id, "Account", list));
        } catch (final Exception e) {
            e.printStackTrace();
        }
    }
...
//					byte buffer[] = doc.getFiles().getBytes(1,(int)adress.getFiles().length());
    ...
    public void dataShowDoc() {
        if (this.actStack.getAccountstackdocses().size() > 0) {
            EBISystem.getCRMModule().getAccountPane().tabModDoc.data = new Object[this.actStack.getAccountstackdocses().size()][4];
            final Iterator itr = this.actStack.getAccountstackdocses().iterator();
            int i = 0;
            while (itr.hasNext()) {
                final Accountstackdocs obj = (Accountstackdocs) itr.next();
                EBISystem.getCRMModule().getAccountPane().tabModDoc.data[i][0] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getAccountPane().tabModDoc.data[i][1] = EBISystem.getInstance().getDateToString(obj.getCreateddate()) == null ? "" : EBISystem.getInstance().getDateToString(obj.getCreateddate());
                EBISystem.getCRMModule().getAccountPane().tabModDoc.data[i][2] = obj.getCreatedfrom() == null ? "" : obj.getCreatedfrom();
                EBISystem.getCRMModule().getAccountPane().tabModDoc.data[i][3] = obj.getAccountdocid();

                i++;
            }
        } else {
            EBISystem.getCRMModule().getAccountPane().tabModDoc.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", ""}};
        }
        EBISystem.getCRMModule().getAccountPane().tabModDoc.fireTableDataChanged();
    }
    ...
    public void dataCalculateTax(final int idCreditDebit) {
        try {
            ...
            if (it.hasNext()) {
                ...
                double dVal = 0.0;
                if (actcd.getCreditdebittype() != EBICRMAccountStack.DEBIT) {
                    try {
                ...
                EBISystem.getCRMModule().getAccountPane().accountDebitCreditType = actcd.getCreditdebittype();
                EBISystem.getCRMModule().getAccountPane().accountDebitTaxName = actcd.getCreditdebittaxtname();

                if (actcd.getCreditdebittype() == EBICRMAccountStack.DEBIT) {

                    EBISystem.getCRMModule().getAccountPane().showDebitID = actcd.getAccountstackcdid();
                    ...
                    EBISystem.getCRMModule().getAccountPane().showCreditID = actcd.getAccountstackcdid();
    ...
    public void dataShowCreditDebitExt(final int type) {
        ...
        try {
            ...
            if (query.list().size() > 0) {
                EBISystem.getCRMModule().getAccountPane().creditDebitMod.data = new Object[query.list().size()][3];
                final Iterator it = query.iterate();
                int i = 0;
                while (it.hasNext()) {
                    final Accountstackcd acCD = (Accountstackcd) it.next();
                    EBISystem.getCRMModule().getAccountPane().creditDebitMod.data[i][0] = acCD.getCreditdebitnumber() == null ? "" : acCD.getCreditdebitnumber();
                    EBISystem.getCRMModule().getAccountPane().creditDebitMod.data[i][1] = acCD.getCreditdebitname() == null ? "" : acCD.getCreditdebitname();
                    EBISystem.getCRMModule().getAccountPane().creditDebitMod.data[i][2] = acCD.getAccountstackcdid();
                    i++;
                }
            } else {
                EBISystem.getCRMModule().getAccountPane().creditDebitMod.data = new Object[][]{
                        {EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), ""}};
            }
            EBISystem.getCRMModule().getAccountPane().creditDebitMod.fireTableDataChanged();
        } catch (final HibernateException e) {
            e.printStackTrace();
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    public void dataShowCreditDebit() {
        ...
        try {
            ...
            if (query.list().size() > 0) {
                ...
                EBISystem.getCRMModule().getAccountPane().creditDebitMod.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), ""}};
            }
            EBISystem.getCRMModule().getAccountPane().creditDebitMod.fireTableDataChanged();
        } catch (final HibernateException e) {
            e.printStackTrace();
