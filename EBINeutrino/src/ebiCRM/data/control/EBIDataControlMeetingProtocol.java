...
public class EBIDataControlMeetingProtocol {
    ...
    public EBIDataControlMeetingProtocol() {
    ...
    public boolean dataStore() {
        try {
            ...
            if (isEdit == true) {
                ...
                meetingProtocol.setCompany(EBISystem.getInstance().company);
            }
            ...
            if (!this.meetingProtocol.getCompanymeetingcontactses().isEmpty()) {
                ...
                while (iter.hasNext()) {
                    ...
                    meetingProtocol.getCompanymeetingcontactses().add(cont);
            ...
            if (!this.meetingProtocol.getCompanymeetingdocs().isEmpty()) {
                ...
                while (iter.hasNext()) {
                    ...
                    meetingProtocol.getCompanymeetingdocs().add(doc);
                    EBISystem.hibernate().session("EBICRM_SESSION").saveOrUpdate(doc);
                }
            }

            EBISystem.getInstance().getDataStore("MeetingCall", "ebiSave");
            EBISystem.getInstance().company.getCompanymeetingprotocols().add(meetingProtocol);
    ...
    public void dataCopy(final int id) {
        ...
        try {
            if (EBISystem.getInstance().company.getCompanymeetingprotocols().size() > 0) {
                Companymeetingprotocol mProtocol = null;
                for (Companymeetingprotocol metObj : EBISystem.getInstance().company.getCompanymeetingprotocols()) {
                ...
                EBISystem.getInstance().company.getCompanymeetingprotocols().add(mPro);
                EBISystem.hibernate().transaction("EBICRM_SESSION").commit();
                ...
                EBISystem.gui().table("companyMeetings", "MeetingCall").
                        changeSelection(EBISystem.gui().table("companyMeetings", "MeetingCall").
                                convertRowIndexToView(EBISystem.getCRMModule().dynMethod.
                                        getIdIndexFormArrayInATable(EBISystem.getCRMModule().getMeetingProtocol().tableModel.data, 4, mPro.getMeetingprotocolid())), 0, false, false);
            }
        } catch (final HibernateException e) {
            e.printStackTrace();
        } catch (final Exception e) {
    ...
    public void dataEdit(final int id) {
        if (EBISystem.getInstance().company.getCompanymeetingprotocols().size() > 0) {
            ...
            EBISystem.gui().table("companyMeetings", "MeetingCall").
                    changeSelection(EBISystem.gui().table("companyMeetings", "MeetingCall").
                            convertRowIndexToView(EBISystem.getCRMModule().dynMethod.
                                    getIdIndexFormArrayInATable(EBISystem.getCRMModule().getMeetingProtocol().tableModel.data, 4, id)), 0, false, false);
        } else {
    ...
    public void dataDelete(final int id) {
        if (EBISystem.getInstance().company.getCompanymeetingprotocols().size() > 0) {
            ...
            EBISystem.getInstance().company.getCompanymeetingprotocols().remove(meetingProtocol);
    ...
    public void dataShow() {
        ...
        final int size = EBISystem.getInstance().company.getCompanymeetingprotocols().size();
        if (size > 0) {
            EBISystem.getCRMModule().getMeetingProtocol().tableModel.data = new Object[size][5];
            final Iterator<Companymeetingprotocol> iter = EBISystem.getInstance().company.getCompanymeetingprotocols().iterator();
            int i = 0;
            while (iter.hasNext()) {
                final Companymeetingprotocol obj = iter.next();
                EBISystem.getCRMModule().getMeetingProtocol().tableModel.data[i][0] = obj.getMetingdate() == null ? "" : EBISystem.getInstance().getDateToString(obj.getMetingdate());
                EBISystem.getCRMModule().getMeetingProtocol().tableModel.data[i][1] = obj.getMeetingsubject() == null ? "" : obj.getMeetingsubject();
                EBISystem.getCRMModule().getMeetingProtocol().tableModel.data[i][2] = obj.getMeetingtype() == null ? "" : obj.getMeetingtype();
                EBISystem.getCRMModule().getMeetingProtocol().tableModel.data[i][3] = obj.getProtocol() == null ? "" : obj.getProtocol();
                EBISystem.getCRMModule().getMeetingProtocol().tableModel.data[i][4] = obj.getMeetingprotocolid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getMeetingProtocol().tableModel.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", ""}};
        }
        EBISystem.getCRMModule().getMeetingProtocol().tableModel.fireTableDataChanged();
    ...
    public void dataNew() {
        meetingProtocol = new Companymeetingprotocol();
        EBISystem.getCRMModule().getMeetingProtocol().initialize(false);
    ...
    private void createHistory(final Company com) throws Exception {
        ...
        EBISystem.getCRMModule().hcreator.setDataToCreate(new EBICRMHistoryDataUtil(com.getCompanyid(), "Meeting", list));
    ...
    public void dataShowContact() {
        if (!this.meetingProtocol.getCompanymeetingcontactses().isEmpty()) {
            EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data = new Object[this.meetingProtocol.getCompanymeetingcontactses().size()][9];
            ...
            while (itr.hasNext()) {
                ...
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][0] = obj.getPosition() == null ? "" : obj.getPosition();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][1] = obj.getGender() == null ? "" : obj.getGender();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][2] = obj.getSurname() == null ? "" : obj.getSurname();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][3] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][4] = obj.getPhone() == null ? "" : obj.getPhone();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][5] = obj.getMobile() == null ? "" : obj.getMobile();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][6] = obj.getEmail() == null ? "" : obj.getEmail();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][7] = obj.getDescription() == null ? "" : obj.getDescription();
                EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data[i][8] = obj.getMeetingcontactid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", "", "", ""}};
        }
        EBISystem.getCRMModule().getMeetingProtocol().tabModelContact.fireTableDataChanged();
    }
    ...
    public void dataShowDoc() {
        if (!this.meetingProtocol.getCompanymeetingdocs().isEmpty()) {
            EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data = new Object[this.meetingProtocol.getCompanymeetingdocs().size()][4];
            ...
            while (itr.hasNext()) {
                ...
                EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data[i][0] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data[i][1] = EBISystem.getInstance().getDateToString(obj.getCreateddate()) == null ? "" : EBISystem.getInstance().getDateToString(obj.getCreateddate());
                EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data[i][2] = obj.getCreatedfrom() == null ? "" : obj.getCreatedfrom();
                EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data[i][3] = obj.getMeetingdocid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", ""}};
        }
        EBISystem.getCRMModule().getMeetingProtocol().tabmeetingDoc.fireTableDataChanged();
    }

    public Set<Companymeetingprotocol> getMeetingPlist() {
        return EBISystem.getInstance().company.getCompanymeetingprotocols();
    }
    ...
    private String getMeetingNamefromId(final int id) {
        String name = "";
        final Iterator iter = EBISystem.getInstance().company.getCompanymeetingprotocols().iterator();
        while (iter.hasNext()) {
