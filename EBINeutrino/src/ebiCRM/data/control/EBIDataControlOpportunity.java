...
public class EBIDataControlOpportunity {
    ...
    public EBIDataControlOpportunity() {
    ...
    public boolean dataStore() {
        ...
        try {
            ...
            if (isEdit == false) {
                ...
                opportunity.setCompany(EBISystem.getInstance().company);
            } else {
            ...
            EBISystem.getInstance().company.getCompanyopportunities().add(opportunity);
    ...
    public void dataCopy(final int id) {
        ...
        try {
        	...
        	if (EBISystem.getInstance().company.getCompanyopportunities().size() > 0) {
        		
        		Companyopportunity opp = null;
        		for(Companyopportunity oprtObj : EBISystem.getInstance().company.getCompanyopportunities()) {
                ...
                EBISystem.getInstance().company.getCompanyopportunities().add(opnew);
                EBISystem.hibernate().transaction("EBICRM_SESSION").commit();
                ...
                EBISystem.gui().table("companyOpportunityTable", "Opportunity").
                        changeSelection(EBISystem.gui().table("companyOpportunityTable", "Opportunity").
                                convertRowIndexToView(EBISystem.getCRMModule().dynMethod.
                                        getIdIndexFormArrayInATable(EBISystem.getCRMModule().getOpportunityPane().tabModel.data, 7, opnew.getOpportunityid())), 0, false, false);
    ...
    public void createOfferFromOpportunity(final int id) {
        EBISystem.getCRMModule().getOfferPane().dataControlOffer.dataNew();
        if (EBISystem.getInstance().company.getCompanyopportunities().size() > 0) {
        	Companyopportunity opp =  null;
        	for(Companyopportunity oprtObj : EBISystem.getInstance().company.getCompanyopportunities()) {
        		if(oprtObj.getOpportunityid() == id) {
            ...
            EBISystem.getCRMModule().getOfferPane().dataControlOffer.opportunityID = id;
            EBISystem.getCRMModule().ebiContainer.setSelectedTab(EBISystem.getInstance().getIEBIContainerInstance().getIndexByTitle(EBISystem.i18n("EBI_LANG_C_OFFER")));
            ...
            if (opp.getCompanyopportunitycontacts().size() > 0) {
                ...
                if (EBIExceptionDialog.getInstance(EBISystem.i18n("EBI_LANG_WOULD_YOU_IMPORT_CONTACT_DATA")).Show(EBIMessage.INFO_MESSAGE_YESNO) == true) {
                    ...
                    while (it.hasNext()) {
                        ...
                        EBISystem.getCRMModule().getOfferPane().dataControlOffer.getCompOffer().getCompanyofferreceivers().add(offRec);
                    }
                    EBISystem.getCRMModule().getOfferPane().dataControlOffer.dataShowReceiver();
                }
            }
            if (opp.getCompanyopporunitydocses().size() > 0) {
                if (EBIExceptionDialog.getInstance(EBISystem.i18n("EBI_LANG_WOULD_YOU_IMPORT_DOCUMENT")).Show(EBIMessage.INFO_MESSAGE_YESNO) == true) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        EBISystem.getCRMModule().getOfferPane().dataControlOffer.getOfferDocList().add(docs);
                    }
                    EBISystem.getCRMModule().getOfferPane().dataControlOffer.dataShowDoc();
                }
            }
        } else {
            EBIExceptionDialog.getInstance(EBISystem.i18n("EBI_LANG_C_RECORD_NOT_FOUND")).Show(EBIMessage.INFO_MESSAGE);
    ...
    public void dataDelete(final int id) {
        if (EBISystem.getInstance().company.getCompanyopportunities().size() > 0) {
            ...
            EBISystem.hibernate().transaction("EBICRM_SESSION").commit();
            EBISystem.getInstance().company.getCompanyopportunities().remove(opp);
        }
    }
    ...
    public void dataShow() {
        ...
        final int size = EBISystem.getInstance().company.getCompanyopportunities().size();

        if (size > 0) {
            EBISystem.getCRMModule().getOpportunityPane().tabModel.data = new Object[size][8];
            final Iterator<Companyopportunity> iter = EBISystem.getInstance().company.getCompanyopportunities().iterator();
            int i = 0;
            ...
            while (iter.hasNext()) {
                ...
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][0] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][1] = obj.getSalestage() == null ? "" : obj.getSalestage();
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][2] = obj.getProbability() == null ? "" : obj.getProbability();
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][3] = obj.getBusinesstype() == null ? "" : obj.getBusinesstype();
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][4] = currency.format(obj.getOpportunityvalue()).equals("0") ? "" : currency.format(obj.getOpportunityvalue());
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][5] = obj.getIsclose() == null ? "" : obj.getIsclose();
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][6] = EBISystem.getInstance().getDateToString(obj.getClosedate()) == null ? "" : EBISystem.getInstance().getDateToString(obj.getClosedate());
                EBISystem.getCRMModule().getOpportunityPane().tabModel.data[i][7] = obj.getOpportunityid();
                i++;
            }
        } else {
        	EBISystem.getCRMModule().getOpportunityPane().tabModel.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", "", "", ""}};
        }
        EBISystem.getCRMModule().getOpportunityPane().tabModel.fireTableDataChanged();
    ...
    public void dataNew() {
        opportunity = new Companyopportunity();
        EBISystem.getCRMModule().getOpportunityPane().initialize(false);
    ...
    public void showOpportunityContacts() {
        if (this.opportunity.getCompanyopportunitycontacts().size() > 0) {
        	EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data = new Object[this.opportunity.getCompanyopportunitycontacts().size()][9];
            ...
            while (itr.hasNext()) {
                ...
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][0] = obj.getPosition() == null ? "" : obj.getPosition();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][1] = obj.getGender() == null ? "" : obj.getGender();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][2] = obj.getSurname() == null ? "" : obj.getSurname();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][3] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][4] = obj.getPhone() == null ? "" : obj.getPhone();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][5] = obj.getMobile() == null ? "" : obj.getMobile();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][6] = obj.getEmail() == null ? "" : obj.getEmail();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][7] = obj.getDescription() == null ? "" : obj.getDescription();
                EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data[i][8] = obj.getOpportunitycontactid();
                i++;
            }
        } else {
        	EBISystem.getCRMModule().getOpportunityPane().tabModelContact.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", "","",""}};
        }
        EBISystem.getCRMModule().getOpportunityPane().tabModelContact.fireTableDataChanged();
    }
    ...
    public void dataShowDoc() {
        ...
        if (this.opportunity.getCompanyopporunitydocses().size() > 0) {
        	EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data = new Object[this.opportunity.getCompanyopporunitydocses().size()][4];

            final Iterator itr = this.opportunity.getCompanyopporunitydocses().iterator();
            int i = 0;
            while (itr.hasNext()) {
                final Companyopporunitydocs obj = (Companyopporunitydocs) itr.next();
                EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data[i][0] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data[i][1] = EBISystem.getInstance().getDateToString(obj.getCreateddate()) == null ? "" : EBISystem.getInstance().getDateToString(obj.getCreateddate());
                EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data[i][2] = obj.getCreatedfrom() == null ? "" : obj.getCreatedfrom();
                EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data[i][3] = obj.getDocid();
                i++;
            }
        } else {
        	EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", ""}};
        }
        EBISystem.getCRMModule().getOpportunityPane().tabOpportunityDoc.fireTableDataChanged();
    }
    ...
    private void createHistory(final Company com) {
        ...
        try {
            EBISystem.getCRMModule().hcreator.setDataToCreate(new EBICRMHistoryDataUtil(com.getCompanyid(), "Opportunity", list));
    ...
    public Set<Companyopportunity> getOppportunityList() {
        return EBISystem.getInstance().company.getCompanyopportunities();
    }
    ...
    private String getOpportunityNamefromId(final int id) {
        ...
        final Iterator iter = EBISystem.getInstance().company.getCompanyopportunities().iterator();
