...
public class EBIDataControlProduct {
                ...
                EBISystem.gui().table("companyProductTable", "Product").
                        changeSelection(EBISystem.gui().table("companyProductTable", "Product").
                                convertRowIndexToView(EBISystem.getCRMModule().dynMethod.
                                        getIdIndexFormArrayInATable(EBISystem.getCRMModule().getEBICRMProductPane().productModel.data, 5, pnew.getProductid())), 0, false, false);
        ...
        try {
            ...
            if (iter.hasNext()) {
                ...
                EBISystem.gui().table("companyProductTable", "Product").
                        changeSelection(EBISystem.gui().table("companyProductTable", "Product").
                                convertRowIndexToView(EBISystem.getCRMModule().dynMethod.
                                        getIdIndexFormArrayInATable(EBISystem.getCRMModule().getEBICRMProductPane().productModel.data, 5, id)), 0, false, false);
                EBISystem.hibernate().transaction("EBIPRODUCT_SESSION").commit();
    ...
    public void dataShow() {
        ...
        try {
            ...
            if (set != null) {
                set.last();
                EBISystem.getCRMModule().getEBICRMProductPane().productModel.data = new Object[set.getRow()][6];
                if (set.getRow() > 0) {
                    set.beforeFirst();
                    int i = 0;
                    while (set.next()) {
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][0] = set.getString("PRODUCTNR") == null ? "" : set.getString("PRODUCTNR");
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][1] = set.getString("PRODUCTNAME") == null ? "" : set.getString("PRODUCTNAME");
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][2] = set.getString("CATEGORY") == null ? "" : set.getString("CATEGORY");
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][3] = set.getString("TYPE") == null ? "" : set.getString("TYPE");
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][4] = set.getString("DESCRIPTION") == null ? "" : set.getString("DESCRIPTION");
                        EBISystem.getCRMModule().getEBICRMProductPane().productModel.data[i][5] = set.getInt("PRODUCTID");
                        i++;
                    }
                } else {
                    EBISystem.getCRMModule().getEBICRMProductPane().productModel.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", ""}};
                }
            } else {
                EBISystem.getCRMModule().getEBICRMProductPane().productModel.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", "", "", "", ""}};
            }
            ...
            EBISystem.getCRMModule().getEBICRMProductPane().productModel.fireTableDataChanged();
        }
    ...
    public void dataNew() {
        product = new Crmproduct();
        EBISystem.getCRMModule().getEBICRMProductPane().initialize(false);
    ...
    public void dataShowDoc() {
        if (this.product.getCrmproductdocses().size() > 0) {
            EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data = new Object[this.product.getCrmproductdocses().size()][4];
            ...
            while (itr.hasNext()) {
                ...
                EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data[i][0] = obj.getName() == null ? "" : obj.getName();
                EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data[i][1] = EBISystem.getInstance().getDateToString(obj.getCreateddate()) == null ? "" : EBISystem.getInstance().getDateToString(obj.getCreateddate());
                EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data[i][2] = obj.getCreatedfrom() == null ? "" : obj.getCreatedfrom();
                EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data[i][3] = obj.getProductdocid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), "", ""}};
        }
        EBISystem.getCRMModule().getEBICRMProductPane().tabModDoc.fireTableDataChanged();
    }
    ...
    public void dataShowDependency() {
        if (product.getCrmproductdependencies().size() > 0) {
            EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.data = new Object[product.getCrmproductdependencies().size()][3];
            final Iterator iter = product.getCrmproductdependencies().iterator();
            int i = 0;
            while (iter.hasNext()) {
                final Crmproductdependency dip = (Crmproductdependency) iter.next();
                EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.data[i][0] = dip.getProductnr();
                EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.data[i][1] = dip.getProductname();
                EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.data[i][2] = dip.getDependencyid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), ""}};
        }
        EBISystem.getCRMModule().getEBICRMProductPane().productDependencyModel.fireTableDataChanged();
    }
    ...
    public void dataShowDimension() {
        if (this.product.getCrmproductdimensions().size() > 0) {
            EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.data = new Object[this.product.getCrmproductdimensions().size()][3];
            final Iterator iter = this.product.getCrmproductdimensions().iterator();
            int i = 0;
            while (iter.hasNext()) {
                final Crmproductdimension dim = (Crmproductdimension) iter.next();
                EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.data[i][0] = dim.getName();
                EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.data[i][1] = dim.getValue();
                EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.data[i][2] = dim.getDimensionid();
                i++;
            }
        } else {
            EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.data = new Object[][]{{EBISystem.i18n("EBI_LANG_PLEASE_SELECT"), ""}};
        }

        EBISystem.getCRMModule().getEBICRMProductPane().productModelDimension.fireTableDataChanged();
    }
