...
public class EBIMeetingAddContactDialog {
    ...
    public void setVisible() {
        ...
        EBISystem.gui().label("email", "addNewContactDialog").setIcon(new ImageIcon(getClass().getClassLoader().getResource("mail.png")));
        ...
        EBISystem.gui().button("newContact", "addNewContactDialog").setIcon(new ImageIcon(getClass().getClassLoader().getResource("new.png")));
        ...
        EBISystem.gui().button("searchContact", "addNewContactDialog").setIcon(new ImageIcon(getClass().getClassLoader().getResource("find.png")));
            ...
            public void actionPerformed(final java.awt.event.ActionEvent e) {
                ...
                if (isMeeting) {
                    if (isEdit) {
                        ...
                        EBISystem.getCRMModule().getMeetingProtocol().dataMeetingControl.addContact(EBIMeetingAddContactDialog.this, contact);
                    } else {
                        final Companymeetingcontacts contact = new Companymeetingcontacts();
                        contact.setMeetingcontactid((EBISystem.getCRMModule().getMeetingProtocol().dataMeetingControl.getMeetingContactlist().size() + 1) * -1);
                    ...
                    if (isEdit) {
                        ...
                        EBISystem.getCRMModule().getOpportunityPane().dataOpportuniyControl.addContact(EBIMeetingAddContactDialog.this, ocontact);
                    } else {
                        final Companyopportunitycontact contact = new Companyopportunitycontact();
                        contact.setOpportunitycontactid((EBISystem.getCRMModule().getOpportunityPane().dataOpportuniyControl.getOpportunity().getCompanyopportunitycontacts().size() + 1) * -1);
                        contact.setCreateddate(new Date());
                        contact.setPos(EBISystem.gui().getCheckBox("mainContact", "addNewContactDialog").isSelected() ? 1 : 0);
                        contact.setCreatedfrom(EBISystem.ebiUser);
                        EBISystem.getCRMModule().getOpportunityPane().dataOpportuniyControl.addContact(EBIMeetingAddContactDialog.this, contact);
                    }
                }
                EBISystem.gui().dialog("addNewContactDialog").setVisible(false);
            }
