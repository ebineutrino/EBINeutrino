...
public class EBICRMInvoice {
    ...
    public EBIDataControlInvoice dataControlInvoice = new EBIDataControlInvoice();;
			...
			public void actionPerformed(final ActionEvent e) {
                if (!EBISystem.gui().combo("categoryText", "Invoice").getSelectedItem()
                    ...
                    if (!dataControlInvoice.isEdit) {
                        final Object[] obj = EBISystem.getCRMModule().dynMethod.getInternNumber(EBISystem.gui()
    ...
    public void initialize(boolean reload) {
        ...
        EBISystem.gui().combo("genderText", "Invoice").setModel(new DefaultComboBoxModel(EBICRMContactPane.gendersList));
    ...
    public void historyInvoice(){
        new EBICRMHistoryView(EBISystem.getCRMModule().hcreator.retrieveDBHistory(
            ...
            public void actionPerformed(final ActionEvent e) {
                if (!EBISystem.i18n("EBI_LANG_PLEASE_SELECT").equals(EBISystem.gui().combo("templateText", "sendEMailMessage").getSelectedItem().toString())){
                    EBISystem.gui().getEditor("MessageAreaText", "sendEMailMessage")
                            .setText(EBISystem.getCRMModule().dynMethod.getEMailTemplate(EBISystem.gui().combo("templateText", "sendEMailMessage").getSelectedItem().toString()));
                }
            }
        });
        ...
        if(spl != null) {
            if (spl[0].equals(EBISystem.i18n("EBI_LANG_C_SERVICE"))) {
                if (dataControlInvoice.loadCompanyService(Integer.parseInt(spl[1].trim()))) {
                    ...
                    EBISystem.getCRMModule().getServicePane().remoteEditService(Integer.parseInt(spl[1].trim()));
                } else {
                ...
                if (dataControlInvoice.loadCompanyOrder(Integer.parseInt(spl[1].trim()))) {
                    ...
                    EBISystem.getCRMModule().getOrderPane().editOrderRemote(Integer.parseInt(spl[1].trim()));
                } else {
                    EBIExceptionDialog.getInstance(EBISystem.i18n("EBI_LANG_C_RECORD_NOT_FOUND"))
                            .Show(EBIMessage.ERROR_MESSAGE);
                }
