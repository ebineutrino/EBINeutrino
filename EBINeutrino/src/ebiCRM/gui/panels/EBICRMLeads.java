...
public class EBICRMLeads {

	private EBIDataControlLeads controlLeads = null;
	public int selectedRow = -1;
	public EBIAbstractTableModel tabModel = null;

	public EBICRMLeads() {
		controlLeads = new EBIDataControlLeads();
	}
	...
	public void initialize() {
		...
		EBISystem.gui().combo("genderText", "Leads").setModel(new DefaultComboBoxModel(EBICRMContactPane.gendersList));
		EBISystem.gui().combo("genderText", "Leads").setEditable(true);
		EBISystem.gui().combo("classificationText", "Leads").setModel(new DefaultComboBoxModel(EBICRMCompanyPane.classification));
	...
	public void deleteLead() {
		...
		if (EBIExceptionDialog.getInstance(EBISystem.i18n("EBI_LANG_MESSAGE_DELETE_RECORD")).Show(EBIMessage.WARNING_MESSAGE_YESNO) == true) {
			...
			if (EBISystem.getInstance().company != null) {
				EBISystem.getCRMModule().resetUI(false, true);
			}
	...
	public void openCompany(){
		...
		EBISystem.getCRMModule().createUI(Integer.parseInt(tabModel.data[selectedRow][11].toString()), true);
