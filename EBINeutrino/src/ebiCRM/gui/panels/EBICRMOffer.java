...
public class EBICRMOffer {
    ...
    public EBIDataControlOffer dataControlOffer = new EBIDataControlOffer();
    ...
    public void editOffer() {
        if(EBISystem.i18n("EBI_LANG_PLEASE_SELECT").
                equals(tabModoffer.data[selectedOfferRow][0].toString())){return;}
    ...
    public void mailOffer() {
        ...
        EBISystem.gui().combo("templateText", "sendEMailMessage").setModel(new DefaultComboBoxModel(EBISystem.getCRMModule().dynMethod.getEMailTemplateNames()));
            ...
            public void actionPerformed(final ActionEvent e) {
                if (!EBISystem.i18n("EBI_LANG_PLEASE_SELECT").equals(
                    ...
                    EBISystem.gui().getEditor("MessageAreaText", "sendEMailMessage").setText(
                            EBISystem.getCRMModule().dynMethod.getEMailTemplate(
    ...
    public void historySearch(){
        new EBICRMHistoryView(EBISystem.getCRMModule().hcreator.retrieveDBHistory(EBISystem.getInstance().company.getCompanyid(), "Offer")).setVisible();
    }

    public void searchOpportunity(){
        final EBIOpportunitySelectionDialog dialog = new EBIOpportunitySelectionDialog(EBISystem.getCRMModule().getOpportunityPane().dataOpportuniyControl.getOppportunityList());
        dialog.setVisible();
    ...
    public void showOpportunity(){
        if (!"".equals(EBISystem.gui().textField("offerOpportunityText", "Offer").getText())) {
            EBISystem.getCRMModule().ebiContainer.setSelectedTab(EBISystem.getInstance().getIEBIContainerInstance().getIndexByTitle(EBISystem.i18n("EBI_LANG_C_OPPORTUNITY")));
            EBISystem.getCRMModule().getOpportunityPane().editOpportunity();
        } else {
