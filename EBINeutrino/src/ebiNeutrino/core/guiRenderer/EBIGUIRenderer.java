...
import ebiNeutrino.core.gui.component.EBITableExt;
import ebiNeutrino.core.gui.lookandfeel.MoodyBlueTheme;
import ebiNeutrino.core.reflection.EBIReflect;
...
public final class EBIGUIRenderer implements IEBIGUIRenderer {
    ...
    public EBIGUIRenderer(final EBIMain main) {
        ...
        gse = new GroovyScriptEngine(new URL[]{getClass().getClassLoader().getResource("MODULES/")});
    }
            ...
            while (ix.hasNext()) {
                ...
                if (bx.getType().toLowerCase().equals("includetoolbar")) {
                    if (EBISystem.getInstance().iuserRights.isAdministrator()) {
                        addXMLToolBarGUI(bx);
                    } else if (EBISystem.moduleToView.contains(bx.getPath())) {
                        addXMLToolBarGUI(bx);
                    }
                } else {
                    if (EBISystem.getInstance().iuserRights.isAdministrator()) {
    ...
    public final void loadGUI(final String path) {
        if (EBISystem.getInstance().iuserRights.isAdministrator()) {
            loadGUIExt("MODULES/" + path);
            canShow = true;
        } else if (EBISystem.moduleToView.contains(path)) {
            loadGUIExt("MODULES/" + path);
            canShow = true;
        }
    }
    ...
    public final void renderGUI(final Object obj, final List<EBIGUIWidgetsBean> tmpList) {
        ...
        if (obj == null) {
            ...
            if (list.getIcon() == null) {
                panel.setModuleIcon(new ImageIcon(getClass().getClassLoader().getResource("appIcon.png")));
            } else {
    ...
    @Override
    public final boolean existPackage(final String name) {
