...
public class EBIAllertTimerDialog {
    ...
    public EBIAllertTimerDialog() {
        nameSpace = EBISystem.builder().loadGUI("CRMDialog/timerDialog.xml");
        initializeAction();
    }
    ...
    public void setVisible(final int actId, final int compId, final String taskName,
        ...
        companyId = compId;
        activityId = actId;
        EBISystem.builder().textArea("messageViewText", nameSpace).setText(taskName + "\n" + formatter.format(dueDate) + EBISystem.i18n("EBI_LANG_DURATION") + " " + duration + " Min \n\n" + message);
        EBISystem.builder().label("pictureAlert", nameSpace).setIcon(EBISystem.getInstance().getIconResource("Warning.png"));
        EBISystem.builder().showGUI();
    }

    public void initializeAction() {
        EBISystem.builder().dialog(nameSpace).setHaveSerial(true);
        EBISystem.builder().button("closeDialog", nameSpace).addActionListener(new ActionListener() {
            @Override
        ...
        EBISystem.builder().button("openTask", nameSpace).addActionListener(new ActionListener() {
    ...
    private void setActionforDialog() {
        if (EBISystem.builder().getCheckBox("setAsDone", nameSpace).isSelected()) {
            try {
        ...
        EBISystem.builder().dialog(nameSpace).setVisible(false);
    }
}
